...
export default class {
  constructor(platform, priAcc, accessory) {
    ...
    if (this.hasColourSupport) {
      ...
      // the same colour made paler or richer - reached nothing and the bulb
    ...
    if (this.hasCTempSupport) {
      ...
      if (this.alShift !== -1) {
        ...
        // characteristics can never run, because it is guarded on this flag -
        // so switching adaptive lighting off left Home still offering it.
  ...
  async internalColourUpdate(value) {
    try {
      ...
      // Don't continue if this colour is the same as before. Saturation is part
      // of that question: comparing hue alone meant a change of saturation at
      // the same hue looked like no change at all, and nothing was sent.
  ...
  externalColourUpdate(valueX, valueY) {
    try {
      ...
      if (this.cacheHue !== h || this.cacheSat !== s) {
        ...
        // Somebody chose this colour outside HomeKit - in the Wemo app, on a
        // remote, or at the wall. Homebridge catches changes made in the Home
        // app itself, but nothing else sees these, so adaptive lighting would
        // carry on and move the bulb off that colour within minutes.
        if (this.alController?.isAdaptiveLightingActive()) {
  ...
  externalCTUpdate(value) {
    try {
      ...
      // A bulb repeating back the temperature adaptive lighting just set can
      // land a few mireds off through the conversion, and treating that as the
      // owner's doing would switch adaptive lighting off moments after it was
      // switched on. Further apart than that and somebody set this temperature
      // themselves, outside HomeKit - which nothing else would notice, so
      // adaptive lighting has to be stopped here.

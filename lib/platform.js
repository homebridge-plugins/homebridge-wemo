  ...
  pluginSetup() {
    ...
    try {
      ...
      listenerServer = createServer((req, res) => {
        ...
        if (req.method === 'NOTIFY' && accessory && remoteIp === accessory.context.ipAddress) {
  ...
  async getDeviceInfo(thisIp, portToTryFirst) {
    ...
    const tryPort = async (port, ipAddress) => {
      try {
        ...
        // Validate this is a legitimate Wemo/Belkin device
        if (!device.deviceType || !device.deviceType.startsWith('urn:Belkin:')) {
          return false
        }

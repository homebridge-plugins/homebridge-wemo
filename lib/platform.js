...
import { parseError, parseSerialNumber, sameIpv4Subnet } from './utils/functions.js'
...
const notifySourceWarned = new Set()
...
export default class {
      ...
      switch (key) {
        ...
        case 'upnpInterval': {
          ...
          if (Number.isNaN(intVal)) {
            ...
            this.config[key] = platformConsts.maxValues[key]
          } else {
    ...
    try {
      ...
      listenerServer = createServer((req, res) => {
        ...
        if (req.method === 'NOTIFY' && accessory) {
          // Only accept a notification from the address the device was discovered on.
          // Anything on the network can reach this listener, so without this check a
          // notification for a known device could be forged from elsewhere. Networks
          // which rewrite source addresses (docker bridge, some NAT) can turn this
          // off with the disableNotifySourceCheck setting.
          const remoteIp = (req.socket.remoteAddress || '').replace('::ffff:', '')
          if (this.config.disableNotifySourceCheck || remoteIp === accessory.context.ipAddress) {
            // A notification from a known device
            req.on('data', (chunk) => {
              body += chunk.toString()
            })
            req.on('end', () => this.httpClient.receiveDeviceUpdate(accessory, body))
          } else if (!notifySourceWarned.has(accessory.UUID)) {
            // Warn once per accessory, so a misconfigured network does not flood the log
            notifySourceWarned.add(accessory.UUID)
            this.log.warn(
              '[%s] %s [%s] %s [%s] - %s.',
              accessory.displayName,
              platformLang.notifyIgnored,
              remoteIp,
              platformLang.notifyIgnoredAs,
              accessory.context.ipAddress,
              platformLang.notifyIgnoredOpt,
            )
          }
        }
        res.writeHead(200)
      ...
      // A failure to listen - the configured port already taken, or a privileged
      // port without the rights to use it - arrives as an 'error' event, not
      // through the listen callback. Node throws an 'error' event that nobody is
      // listening for, so without this the bridge dies on a port clash and
      // restarts into the same clash. The callback below only ever runs on
      // success, and node calls it with no arguments.
      listenerServer.on('error', (err) => {
        this.log.warn('%s: %s.', platformLang.listenerError, parseError(err))
      })

      const logListenerPort = () => {
        this.log.debug('%s [%s].', platformLang.listenerPort, listenerServer.address().port)
      }
      ...
      if (this.config.wemoClient.listen_interface) {
        ...
        listenerServer.listen(this.config.wemoClient.port, this.getLocalInterfaceAddress(), logListenerPort)
      } else {
        // User has not defined a specific network interface to listen on
        listenerServer.listen(this.config.wemoClient.port, logListenerPort)
      }

      // Set up the SSDP client if the user has not specified manual devices only
      if (this.config.mode !== 'manual') {
        // The discovery client's own chatter goes to the debug log, where
        // Homebridge decides whether it is shown. It used to be handed the
        // normal logger only when the plugin thought debug was on, which it
        // could not tell in a child bridge. The wrapper matters: Homebridge's
        // `debug` is not bound to its logger, so passing it bare would throw
        // the first time the discovery client tried to log anything.
        this.config.wemoClient.discover_opts.customLogger = (...args) => this.log.debug(...args)
  ...
  pluginShutdown() {
    try {
      ...
      if (listenerServer) {
        listenerServer.close(() => this.log.debug('%s.', platformLang.listenerClosed))
      }

      // Stop the SSDP client if it's running
      if (ssdpClient) {
        ssdpClient.stop()
        this.log.debug('%s.', platformLang.ssdpStopped)
      }
      ...
      devicesInHB.forEach((accessory) => {
        ...
        // A garage door part-way through opening or closing has a timer waiting to
        // write its settled state. That was left running, so it fired against a
        // torn-down accessory and held the process open until it did.
        if (accessory.control?.movingTimer) {
          clearTimeout(accessory.control.movingTimer)
          accessory.control.movingTimer = false
        }
        if (accessory.client) {
  ...
  async discoverDevices() {
    ...
    // *************************** \\
    // Awaited, unlike the `forEach(async ...)` this used to be, which threw the
    // promises away. The "still awaiting" warning further down then ran before
    // these probes had finished, so a manually configured device that was about
    // to connect could be reported as one that had not.
    // Still concurrent rather than a for..of loop, because each entry is an
    // independent port scan and running them one after another would drag out
    // discovery. Each callback keeps its own try/catch, so one failure cannot
    // reject the whole batch.
    await Promise.all(this.manualDevices.map(async (device) => {
      try {
  ...
  async initialiseDevice(device) {
    try {
      ...
      const context = {
        ...
        cbURL: device.cbURL,
        // The urn device type lets the UI device picker place a device in the
        // right config array
      ...
      if (device.serviceList) {
        ...
        // Device has no services so cannot be set up on this run.
        //
        // The cached accessory used to be unregistered here. A setup document
        // that arrives truncated, or from a device part-way through a reboot,
        // parses fine but has no serviceList - and one of those was enough to
        // take the accessory out of HomeKit along with its room, its scenes and
        // any automations using it. A later restart brings it back as a brand new
        // accessory, so none of that comes back with it. Leave it alone: it shows
        // as No Response until the device answers properly again.
    ...
    Object.entries(interfaces).forEach((entry) => {
      ...
      Object.entries(interfaces[k]).forEach((subEntry) => {
        ...
        if ((address.family === 'IPv4' || address.family === 4) && !address.internal) {
          if (targetNetwork && sameIpv4Subnet(address.address, targetNetwork, address.netmask)) {
